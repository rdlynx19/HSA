...
def main(model_path=None):
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        while viewer.is_running():
            ...
            # Log actuator activity
            actuator_log.append(abs(data.ctrl[actuator_id]) > 1e-3)

            # Measure distance between spring_1a and spring_1c COMs
            pos_a = data.xpos[spring1a_id].copy()
            pos_c = data.xpos[spring1c_id].copy()
    ...
    # Highlight periods when actuator was active
    start = None
